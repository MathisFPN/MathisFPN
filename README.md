# Mathis FRAPPIN

Développeur Full Stack & DevOps — En recherche d'alternance pour septembre 2026  
Master Informatique Lyon / ESISAR Valence

---

## Stack

**Mobile & Frontend**  
Flutter · Vue.js · Swift · Kotlin · Express.js · HTML/CSS

**Backend & Data**  
C# · .NET · Python · SQL (PostgreSQL) · API REST

**DevOps & Infra**  
Docker · Kubernetes · GitLab CI · Garage (S3) · Grafana · InfluxDB · SonarQube

---

## Expérience

**Développeur d'applications — Alternance**  
Département de la Haute-Savoie · 2025–2026

- Application mobile Flutter livrée en production
- Interface d'administration Vue.js + API REST
- Infrastructure : Docker, K8s, GitLab CI, Grafana/InfluxDB, Garage (S3)
- Coordination d'une équipe de 3 personnes

---

## Projets

| Projet | Stack | Description |
|--------|-------|-------------|
| [Pokémon Trainer Manager](https://github.com/MathisFPN/Poke-mon-Trainer-Manager) | Swift | Application iOS de gestion de collection Pokémon |
| [Portfolio](https://mathisfpn.github.io) | TypeScript | Portfolio et vitrine de projets |

---

## Langues

Français natif · Anglais B2 (Cambridge) · Espagnol A2 · Japonais A1

---

📍 Annecy, France  
📬 mathis.frappin@etu.univ-smb.fr  
🔗 [LinkedIn](https://linkedin.com/in/mathis-frappin) · [Portfolio](https://mathisfpn.github.io)
